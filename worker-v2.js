...
function acceptsHtml(r){return(r.headers.get("Accept")||"").includes("text/html")}
...
async function chatRoomSocket(request,env){const a=await auth(request,env);if(!a.success)return new Response(a.error,{status:a.status});const url=new URL(request.url);let room=String(url.searchParams.get("room")||"main").trim();if(!room||room.length>300)return new Response("Invalid room",{status:400});if(room!=="main"){const parts=room.split(":");const kind=parts.shift();const id=parts.join(":");if(kind!=="dm"&&kind!=="group")return new Response("Invalid room",{status:400});if(kind==="dm"){const dm=await env.USERS.get(`chat:dm:${id}`,"json");if(!dm||!dm.members?.includes(a.user.id))return new Response("Forbidden",{status:403})}else{const group=await env.USERS.get(`chat:group:${id}`,"json");if(!group||!group.members?.includes(a.user.id))return new Response("Forbidden",{status:403})}}
const headers=new Headers(request.headers);headers.set("X-Chat-User-Id",a.user.id);headers.set("X-Chat-Username",a.user.username);headers.set("X-Chat-Display-Name",a.user.displayName||"");headers.set("X-Chat-Room",room);const roomId=env.CHAT_ROOM.idFromName(`chat:${room}`);const stub=env.CHAT_ROOM.get(roomId);return stub.fetch(new Request(request,{headers}));}
...
function getToken(r){const cookie=r.headers.get("Cookie")||"";for(const part of cookie.split(";")){const[name,...rest]=part.trim().split("=");if(name==="session"&&rest.length)return rest.join("=")}const a=r.headers.get("Authorization")||"";return a.startsWith("Bearer ")?a.slice(7).trim():null}
